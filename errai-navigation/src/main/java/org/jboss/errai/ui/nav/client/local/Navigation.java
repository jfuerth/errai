  ...
  private <W extends Widget> void show(PageNode<W> toPage, Multimap<String,String> state) {
    ...
    if (currentPage != null) {
      try {
        ...
        Multimap<String, String> oldState = currentPage.pageHiding(contentPanel.getWidget());
        String historyString = HistoryToken.of(currentPage.name(), oldState).toString();

        String poppedToken = History.getToken();
        System.out.println("Going to " + toPage.name() + "; poppedToken = " + poppedToken);
//        History.back();
        History.newItem(historyString, false);
        History.newItem(poppedToken, false);
      }
      finally {
